...
from customers.models import InviteCode, User
...
class EmailVerificationTestCase(TestCase):
    def setUp(self):
        ...
        self.invite_code = InviteCode.objects.create(
            code=InviteCode.generate_code(), is_active=True
        )
    ...
    def test_registration_sends_email(self):
        ...
        response = self.client.post(
            reverse("register"), data=json.dumps(data), content_type="application/json"
        )
    ...
    def test_resend_verification_email(self):
        ...
        # First create a user
        user = User.objects.create_user(email="testuser2@example.com", password="TestPassword123!")
        ...
        self.assertEqual(
            json.loads(response.content)["detail"], "Verification email sent successfully"
        )
    ...
    def test_verification_link_works(self):
        ...
        user = User.objects.create_user(email="testuser4@example.com", password="TestPassword123!")
        ...
        # Verify email
        response = self.client.get(reverse("verify-email") + f"?token={token}")
