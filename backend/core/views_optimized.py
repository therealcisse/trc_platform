...
from .authentication_optimized import OptimizedBearerTokenAuthentication
...
class OptimizedSolveView(APIView):
    """
    Optimized image solving endpoint.

    Optimizations:
    1. Use optimized token auth middleware
    2. Cache settings to avoid DB hits
    3. Defer non-critical operations
    4. Minimize database queries
    """

    authentication_classes = [OptimizedBearerTokenAuthentication]
...
@api_view(["GET"])
@permission_classes([])
def optimized_healthz(request: Request) -> Response:
    """
