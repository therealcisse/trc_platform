...
def print_period_summary(period: BillingPeriod, token_name: str) -> None:
    ...
    if period.payment_status == 'paid':
        print(f"  - Paid At: {period.paid_at.strftime('%Y-%m-%d %H:%M') if period.paid_at else 'N/A'}")
        print(f"  - Amount Paid: ${period.paid_amount_cents / 100:.2f}" if period.paid_amount_cents else "")
    ...
    try:
        ...
        print(f"  - Last Used: {token.last_used_at.strftime('%Y-%m-%d %H:%M') if token.last_used_at else 'Never'}")
    ...
    if total > 0:
        ...
        print(f"  - With Images: {with_images} ({with_images*100/successful:.1f}% of successful)" if successful > 0 else "")
        ...
        daily_stats = (
            requests
            .annotate(date=TruncDate('request_ts'))
            .values('date')
...
def main():
    ...
    users_with_periods = User.objects.filter(
        billing_periods__period_start__year=2025,
        billing_periods__period_start__month__in=[6, 7, 8]
    ).distinct()
    ...
    for user in users_with_periods:
        ...
        try:
            july_period = BillingPeriod.objects.get(
                user=user,
                period_start__year=2025,
                period_start__month=7
            )
            print_period_summary(july_period, "JULY")
        except BillingPeriod.DoesNotExist:
            print(f"\n{'='*60}")
            print(f"July 2025 - No data")
            print(f"{'='*60}")
        ...
        try:
            august_period = BillingPeriod.objects.get(
                user=user,
                period_start__year=2025,
                period_start__month=8
            )
            print_period_summary(august_period, "AUGUST")
        except BillingPeriod.DoesNotExist:
            print(f"\n{'='*60}")
            print(f"August 2025 - No data")
            print(f"{'='*60}")
    ...
    if all_periods:
        ...
        for period in all_periods:
            ...
            print(f"{period.user.email:<30} {period.period_label:<15} {period.payment_status:<10} {current:<8} ${period.total_cost_cents/100:>8.2f}")
