...
def test_current_user_endpoint():
    ...
    if login_response.status_code == 200:
        ...
        if me_response.status_code == 200:
            ...
            print(f"   ✓ /customers/me successful!")
    ...
    if admin_users.exists():
        ...
        admin_login = admin_client.post('/api/customers/login',
                                        data=json.dumps({
        ...
        if me_recheck.status_code == 200:
            ...
            if recheck_data.get('email') == test_user.email:
                print(f"   ✅ Original session still returns correct user: {recheck_data.get('email')}")
            else:
                print(f"   ❌ Session contamination detected!")
                print(f"      Expected: {test_user.email}")
